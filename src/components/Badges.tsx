...
export const STATUS_CONFIG: Record<CharacterStatus, { label: string; className: string }> = {
  ...
  not_owned: { label: 'Non Possédé', className: 'bg-[#1A1A2E]  text-[#8888AA]  border-[#2D2D4E]'  },
}
...
// Ascended badge — affiché en plus du statut, jamais à la place
export function AscendedBadge() {
  return (
    <span className="badge border bg-cyan-900/60 text-cyan-300 border-cyan-600">
      ⬆ Ascended
    </span>
  )
}

// Clickable status — cycles through the 4 statuses
...
export function InlineAscendedBadge({ ascended, onChange }: InlineAscendedProps) {
  ...
  if (!ascended) return (
    <button
      onClick={handleClick}
      title="Cliquer pour marquer comme Ascended"
  ...
  return (
    <button
      onClick={handleClick}
      title="Cliquer pour retirer Ascended"
...
export function PowerColorDot({ couleur }: { couleur: string | null }) {
  ...
  const cls = POWER_COLORS[couleur] ?? 'bg-gray-500'
...
export const STATUS_CONFIG: Record<CharacterStatus, { label: string; className: string }> = {
  max_champed:  { label: 'Max Champé',   className: 'bg-purple-900/60 text-purple-300 border-purple-700'  },
  champed:      { label: 'Champé',       className: 'bg-orange-900/60 text-orange-300 border-orange-700'  },
  ascended:     { label: 'Ascended',     className: 'bg-cyan-900/60   text-cyan-300   border-cyan-600'    },
  rostered:     { label: 'Roster',       className: 'bg-green-900/60  text-green-300  border-green-700'   },
  not_rostered: { label: 'Non Roster',   className: 'bg-gray-800/60   text-gray-400   border-gray-600'    },
  not_owned:    { label: 'Non Possédé',  className: 'bg-[#1A1A2E]     text-[#8888AA]  border-[#2D2D4E]'  },
}
...
export function StatusBadge({ status }: { status: CharacterStatus | null }) {
  ...
  const cfg = STATUS_CONFIG[status]
  return (
    <span className={`badge border ${cfg.className}`}>{cfg.label}</span>
  )
}

// Clickable version — cycles through statuses inline
const STATUS_ORDER: CharacterStatus[] = [
  'max_champed', 'champed', 'ascended', 'rostered', 'not_rostered', 'not_owned'
]
