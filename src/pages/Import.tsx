...
type ImportResult = { section: string; inserted: number; skipped: number; error?: string }
...
// ── Importers ────────────────────────────────────────────────────
async function importCharacters(rows: unknown[][]): Promise<Result> {
  const records = []
  ...
  for (const r of records) {
    const { error } = await supabase.from('characters').insert([r])
    if (error) skipped++   // duplicate or other error → skip
    else inserted++
  }
  return { section: 'Personnages', inserted, skipped }
}

async function importSupports(rows: unknown[][]): Promise<Result> {
  const records = []
  for (const row of rows.slice(1)) {
    const name = clean(row[0])
  ...
  let inserted = 0, skipped = 0
  for (const r of records) {
    const { error } = await supabase.from('supports').insert([r])
    if (error) skipped++
    else inserted++
  }
...
async function importTeams(rows: unknown[][], status: string): Promise<Result> {
  const records = []
  for (const row of rows.slice(1)) {
    const name = clean(row[0])
  ...
  let inserted = 0, skipped = 0
  for (const r of records) {
    const { error } = await supabase.from('teams').insert([r])
    if (error) skipped++
    else inserted++
  }
  const label = status === 'active' ? 'Équipes actives' : 'Équipes à tester'
  return { section: label, inserted, skipped }
}

async function importQuetes(rows: unknown[][]): Promise<Result> {
  const records = []
  for (const row of rows.slice(1)) {
    const nom = clean(row[0])
    if (!nom || nom === 'Quete') continue
    const splitSlot = (val: unknown) => {
      const parts = (clean(val) ?? '').split('\n')
      return { char: parts[0] || null, build: parts[1] || null, support: parts[2] || null }
    }
    const g = splitSlot(row[1]), m = splitSlot(row[2]), d = splitSlot(row[3])
    records.push({
      nom,
  ...
  let inserted = 0, skipped = 0
  for (const r of records) {
    const { error } = await supabase.from('quetes').insert([r])
    if (error) skipped++
    else inserted++
  }
...
async function importGauntlet(rows: unknown[][]): Promise<Result> {
  const records = []
  for (const row of rows.slice(1)) {
    ...
    const g = splitSlot(row[3]), m = splitSlot(row[4]), d = splitSlot(row[5])
    records.push({
  ...
  let inserted = 0, skipped = 0
  for (const r of records) {
    const { error } = await supabase.from('puzzle_gauntlet').insert([r])
    if (error) skipped++
    else inserted++
  }
...
export default function Import() {
  ...
  function toggleAll() {
    ...
    setSelected(Object.fromEntries(SECTIONS.map(s => [s.key, !allOn])))
  }
  ...
  async function runImport() {
    ...
    try {
      ...
      if (selected.characters) {
        const rows = getSheet(wb, 'Characters')
        if (rows) res.push(await importCharacters(rows))
        else res.push({ section: 'Personnages', inserted: 0, skipped: 0, error: 'Feuille "Characters" introuvable' })
      }
      if (selected.supports) {
        const rows = getSheet(wb, 'Supports')
        if (rows) res.push(await importSupports(rows))
        else res.push({ section: 'Supports', inserted: 0, skipped: 0, error: 'Feuille "Supports" introuvable' })
      }
      if (selected.teams) {
        const rows = getSheet(wb, 'Teams Database')
        if (rows) res.push(await importTeams(rows, 'active'))
        else res.push({ section: 'Équipes actives', inserted: 0, skipped: 0, error: 'Feuille "Teams Database" introuvable' })
      }
      if (selected.teams_to_test) {
        const rows = getSheet(wb, 'Teams to Test')
        if (rows) res.push(await importTeams(rows, 'to_test'))
        else res.push({ section: 'Équipes à tester', inserted: 0, skipped: 0, error: 'Feuille "Teams to Test" introuvable' })
      }
      if (selected.quetes) {
        const rows = getSheet(wb, 'Quetes')
        if (rows) res.push(await importQuetes(rows))
        else res.push({ section: 'Quêtes', inserted: 0, skipped: 0, error: 'Feuille "Quetes" introuvable' })
      }
      if (selected.gauntlet) {
        const rows = getSheet(wb, 'Puzzle Gauntlet')
        if (rows) res.push(await importGauntlet(rows))
        else res.push({ section: 'Puzzle Gauntlet', inserted: 0, skipped: 0, error: 'Feuille "Puzzle Gauntlet" introuvable' })
      }
    ...
    <div className="space-y-6 max-w-2xl">
      ...
      <div
        ...
        className={`border-2 border-dashed rounded-xl p-10 text-center cursor-pointer transition-all ${
          dragging
            ? 'border-marvel-red bg-marvel-red/10'
            : file
      ...
      <div className="card space-y-3">
        ...
        <div className="space-y-2">
          {SECTIONS.map(s => (
            <label key={s.key} className="flex items-center gap-3 cursor-pointer group p-2 rounded-lg hover:bg-[#2D2D4E]/40 transition-colors">
              <button
                type="button"
                onClick={() => setSelected(prev => ({ ...prev, [s.key]: !prev[s.key] }))}
                className={`shrink-0 transition-colors ${selected[s.key] ? 'text-marvel-red' : 'text-[#555]'}`}
              >
                {selected[s.key] ? <CheckSquare size={18} /> : <Square size={18} />}
              </button>
              <div className="flex-1">
                <p className={`text-sm font-medium ${selected[s.key] ? 'text-white' : 'text-[#8888AA]'}`}>{s.label}</p>
                <p className="text-xs text-[#555]">{s.description} · feuille « {s.sheet} »</p>
              </div>
            </label>
          ))}
        </div>
      ...
      {results.length > 0 && (
        <div className="card space-y-3">
          ...
          {results.map((r, i) => (
            <div key={i} className={`flex items-start gap-3 p-3 rounded-lg ${r.error ? 'bg-red-900/20 border border-red-800/40' : 'bg-[#0D0D0D]'}`}>
              {r.error
