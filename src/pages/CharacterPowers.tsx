...
function catColor(cat: string | null): string {
  if (!cat) return 'bg-[#52527A] text-[#8888AA] border-[#444]'
  ...
  return 'bg-[#52527A] text-[#8888AA] border-[#444]'
}
        ...
        <div className="absolute z-30 top-full left-0 right-0 mt-1 bg-[#32325A] border border-[#52527A] rounded-lg shadow-xl max-h-52 overflow-y-auto">
          {filtered.map(c => (
            <button key={c.id} type="button"
              ...
              className="w-full text-left px-3 py-2 text-sm hover:bg-[#52527A] flex items-center gap-2">
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <div className="flex gap-1 bg-[#28284A] p-1 rounded-lg">
                ...
                <button onClick={() => setExpanded(isOpen ? null : charId)}
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <button onClick={e => { e.stopPropagation(); openAdd(charId) }}
                      className="text-xs bg-[#52527A] hover:bg-marvel-red/40 text-[#8888AA] hover:text-white px-2 py-1 rounded transition-all">
                      + Pouvoir
                    </button>
                ...
                {isOpen && (
                  <div className="mt-4 pt-4 border-t border-[#52527A] space-y-4">
                    {COULEURS.filter(col => byCouleur[col]).map(col => (
                      <div key={col}>
                        <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-lg border mb-2 ${COULEUR_BORDER[col as Couleur]} bg-[#2A2A45]`}>
                        ...
                        <div className="space-y-2 pl-2">
                          {byCouleur[col].sort((a, b) => (a.cout ?? 0) - (b.cout ?? 0)).map(p => (
                            <div key={p.id} className="bg-[#2A2A45] rounded-lg p-3 flex items-start gap-3 group">
          ...
          <table className="w-full text-xs">
            ...
            <tbody>
              {sortedFiltered.map(p => (
                <tr key={p.id} className="border-b border-[#52527A]/40 hover:bg-[#52527A]/20 align-top">
          ...
          <div className="card w-full max-w-2xl my-4">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="space-y-2">
                  {EFFECTS.map(n => (
                    <div key={n} className="bg-[#2A2A45] rounded-lg p-3 grid grid-cols-2 gap-2">
                      <div>
