...
function InlineLevelCell({ id, level, onSave }: {
  ...
  if (editing) return (
    <input ref={inputRef} type="number" value={val}
      ...
      className="w-16 bg-[#2A2A45] border border-marvel-red rounded px-1 py-0.5 text-center text-white text-xs outline-none" />
  )
  return (
    <button onClick={startEdit} title="Cliquer pour modifier le niveau"
      className="text-[#8888AA] hover:text-white hover:bg-[#52527A] rounded px-2 py-0.5 transition-all min-w-8 text-center">
      {level ?? '—'}
    </button>
  )
}
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2 text-xs text-[#8888AA]">
        <span>Trier par :</span>
        <div className="flex gap-3 bg-[#28284A] px-3 py-1.5 rounded-lg">
      ...
      {loading ? <Spinner /> : viewMode === 'list' ? (
        <div className="card overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-[#52527A] text-[#8888AA]">
            ...
            <tbody>
              {filtered.map(c => (
                <tr key={c.id} className="border-b border-[#52527A]/40 hover:bg-[#52527A]/20">
                  ...
                  <td className="py-2 text-center text-xs text-[#8888AA]">
                    {c.version && <span className="bg-[#52527A] px-1.5 py-0.5 rounded">{c.version}</span>}
                  </td>
          ...
          {Object.entries(grouped).map(([baseName, versions]) => (
            <div key={baseName} className="card">
              ...
              <div className="flex flex-wrap gap-2">
                {versions.map(c => (
                  <div key={c.id} className="flex items-center gap-2 bg-[#2A2A45] rounded-lg px-3 py-2 group">
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="card w-full max-w-md">
            ...
            <div className="space-y-3">
              ...
              {form.base_name && (
                <div className="bg-[#2A2A45] rounded-lg px-3 py-2 flex gap-4 text-xs">
                ...
                <div>
                  ...
                  <button type="button" onClick={() => setForm(f => ({ ...f, ascended: !f.ascended }))}
                    className={`w-full py-2 px-3 rounded-lg border text-sm font-medium transition-all ${
                      form.ascended ? 'bg-cyan-900/60 border-cyan-600 text-cyan-300' : 'bg-[#2A2A45] border-[#52527A] text-[#8888AA] hover:border-cyan-600/50'
                    }`}>
