      ...
      <aside className="w-56 bg-[#12122A] border-r border-[#2D2D4E] flex flex-col">
            ...
            <NavLink key={to} to={to} end={to === '/'}
              className={({ isActive }) =>
                `flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-all ${
                  isActive ? 'bg-marvel-red text-white font-semibold' : 'text-[#8888AA] hover:bg-[#1A1A2E] hover:text-white'
                }`}>
              <Icon size={16} />{label}
            </NavLink>
          ))}
        ...
        <div className="p-3 border-t border-[#2D2D4E]">
          <button onClick={signOut}
            className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm text-[#8888AA] hover:bg-[#1A1A2E] hover:text-white transition-all w-full">
